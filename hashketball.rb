  ...
  :players => [
    ...
    :team_name => 'Charlotte Hornets',
     ...
     :players => [
       ...
       {player_name: 'Bismak Biyombo', number: 0, shoe: 16, 
          points: 12, rebounds: 4, assists: 7, steals: 15, blocks: 15, slam_dunks: 10},
...
def num_points_scored(players_numbers)
  game_hash.values.each do |team|
    team[:players].each do |player|
      return player[:points] if player.has_value?(player_numbers)
    end
  end
end
...
def team_colors(team_name)
  game_hash.values.each do |team_info|
    if team_info.has_value?(team)
      return team.info[:colors].map
    end
  end
end

def team_names
  game_hash.values.map {|value| value[:tream_name]}
end
